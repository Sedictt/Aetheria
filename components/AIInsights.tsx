...
const AIInsights: React.FC<AIInsightsProps> = ({ note, isOpen, onClose, onUpdateNote }) => {
  ...
  return (
    <div className="w-80 bg-stone-50 border-l border-stone-200 h-full overflow-y-auto p-6 shadow-xl absolute right-0 top-0 z-20 md:static md:shadow-none transition-all animate-in slide-in-from-right duration-300">
      ...
      {!hasAnalysis ? (
        ...
        <div className="space-y-6">
          {/* Mood Card */}
          {currentMood && (
            <div className="bg-white p-5 rounded-xl shadow-sm border border-stone-100 overflow-hidden relative group/card">
              <div 
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="relative group/picker mt-1">
                      ...
                      />
                      <div
                          className="w-4 h-4 rounded-full border border-stone-100 shadow-sm transition-transform group-hover/picker:scale-110 cursor-pointer"
                          style={{ backgroundColor: currentMood.color }}
                      />
                      <span className="absolute left-full ml-2 top-1/2 -translate-y-1/2 text-[9px] text-stone-400 opacity-0 group-hover/picker:opacity-100 whitespace-nowrap pointer-events-none transition-opacity bg-stone-50 px-1 py-0.5 rounded border border-stone-200">
                          Customize
                      </span>
                    </div>
                  </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {(note.tags || []).map(tag => (
                <span key={tag} className="group flex items-center gap-1 px-3 py-1 bg-white border border-stone-200 rounded-full text-xs text-stone-600 shadow-sm hover:border-stone-300 transition-colors">
                  #{tag}
                  <button
              ...
              <input 
                  ...
                  placeholder="+ Add tag..."
                  className="text-xs bg-transparent border border-transparent hover:border-stone-200 focus:border-stone-300 focus:bg-white rounded-full px-3 py-1 outline-none text-stone-600 placeholder-stone-400 transition-all w-24 focus:w-32"
              />
            </div>
          </div>
          ...
          {note.aiSummary && (
            <div className="bg-white p-5 rounded-xl shadow-sm border border-stone-100">
              ...
              <p className="text-stone-700 text-sm leading-relaxed">{note.aiSummary}</p>
            </div>
          )}
          ...
          {note.aiReflection && (
            <div className="bg-indigo-50/50 p-5 rounded-xl border border-indigo-100">
              <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider block mb-2">Thought Provoker</span>
              <p className="text-indigo-900 font-serif italic text-sm leading-relaxed">
                "{note.aiReflection}"
              </p>
            </div>
          )}
